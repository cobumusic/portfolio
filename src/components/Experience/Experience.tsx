    ...
    <div className={css.item} data-reveal>
      ...
      <ul className={css.bullets}>
        {item.bullets.map((b, i) => (
          <li className={css.bullet} key={i}>{b}</li>
        ))}
      </ul>
    </div>
  )
...
export default function Experience() {
  ...
  return (
    <section id="experience" className={`section ${css.experience}`} ref={sectionRef}>
      <div className={css.header} data-reveal>
        <p className="section__label">Experience</p>
        <h2 className="section__title">Where I&apos;ve <span>worked</span></h2>
        <p className="section__subtitle">
          From defense tech to AI startups, I&apos;ve built production software across industries that demand both precision and speed.
        </p>
      </div>
